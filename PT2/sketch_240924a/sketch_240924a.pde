...
void draw() {
  ...
  if (grow){
    flamesize += 1;
    if (flamesize > 60) {
      grow = false; //api mengecil setelah mencapai ukuran maksimal
    }
  } else {
    flamesize -= 1;
    if (flamesize < 40) {
      grow = true; //api membesar setelah mencapai ukuran minimal
    }
  }
  
  // Api roket
  fill (255,140,0);
  triangle(180, 470, 220, 470, 200, 470 + flamesize); //Api Utama
  
  fill (255, 255, 0);
  triangle(185, 470, 215, 470, 200, 470 + flamesize); //bayangan api
}
