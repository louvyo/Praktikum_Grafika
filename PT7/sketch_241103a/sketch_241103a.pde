...
color rocketColor = color(170, 170, 250); // Warna awal badan roket
...
void setup() {
  ...
  // Inisialisasi suara roket
  rocketSound = new SoundFile(this, "rocketSound.wav"); // Tambahkan file rocketSound.mp3 di folder sketch
  rocketSound.loop(); // Memutar suara roket secara terus menerus
}
...
void drawRocket3D() {
  ...
  fill(rocketColor); // Warna badan roket bisa berubah saat diklik
...
// Event Mouse: Mengubah warna roket saat diklik
void mousePressed() {
  if (dist(mouseX, mouseY, rocketX, height - rocketY) < 50) { // Radius klik dekat roket
    rocketColor = color(random(255), random(255), random(255)); // Ubah warna roket
  }
}

// Event Keyboard: Menggerakkan roket dengan tombol panah
void keyPressed() {
  if (key == CODED) {
    if (keyCode == UP) {
      rocketY += rocketSpeed;
